...
// Include the database configuration file to establish DB connection
require_once __DIR__ . "/config/database.php";

// Dashboard class extends Database to inherit connection methods
class Dashboard extends Database {
    public function __construct() {     // Constructor automatically connects to the database upon class initialization
        parent::__construct();          // Call parent (Database) constructor
        $this->connect();               // Initialize database connection
    }
    
    // Get total number of categories in the category table
    public function getTotalCategories() {
        $result = $this->conn->query("SELECT COUNT(*) as total FROM tbl_category");
        return $result->fetch_assoc()['total'];     // Return total count value
    }
    
    // Get total number of suppliers in the supplier table
    public function getTotalSuppliers() {
        $result = $this->conn->query("SELECT COUNT(*) as total FROM tbl_supplier");
        return $result->fetch_assoc()['total'];
    }

    // Get total number of products in the product table
    public function getTotalProducts() {
        $result = $this->conn->query("SELECT COUNT(*) as total FROM tbl_product");
        return $result->fetch_assoc()['total'];
    }
    
    // Get the sum of all product quantities (total stock)
    public function getTotalStock() {
        $result = $this->conn->query("SELECT SUM(quantity) as total FROM tbl_product");
        return $result->fetch_assoc()['total'] ?? 0;
    }
    
    // Fetch products that have low stock (quantity < 20), with category and supplier info
...
// Instantiate Dashboard class and retrieve dashboard metrics
...
// Include common layout components (header and sidebar)
            ...
            <div class="row">
                <div class="col-lg-3 col-6">
                    <div class="small-box text-bg-primary">
                        <div class="inner">
                            <!-- Display total categories from backend -->
                                ...
                                <table class="table m-0">
                                    ...
                                    <tbody>
                                        <?php if (!empty($lowStockProducts)): ?>
                                            <!-- Loop through all low stock products -->
